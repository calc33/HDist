...
namespace HDist.Core
{
	public partial class FileList : IEnumerable<FileList.FileEntry>
	{
		...
		/// </returns>
		public bool TryRunShadowCopy(string[] copyFiles)
		{
			if (copyFiles.Length == 0)
			{
				return false;
			}
			string[]? files = GetShadowCopyFiles(copyFiles);
			if (files == null)
			{
				return false;
			}
			...
			string exePath = Path.Combine(tempDir, Path.GetRelativePath(DestinationDirectory, copyFiles[0]));
